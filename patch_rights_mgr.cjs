...
const newRender = `
                {Object.entries(permissions).map(([moduleKey, mods]) => (
                  <div key={moduleKey} className={\`border \${mods.access ? 'border-gray-200' : 'border-gray-300'} rounded-lg overflow-hidden transition-all\`}>
                    <div className={\`px-4 py-3 font-bold capitalize flex justify-between items-center \${mods.access ? 'bg-gray-50 text-gray-800 border-b border-gray-200' : 'bg-gray-100 text-gray-500'}\`}>
                      <span>{moduleKey.replace(/([A-Z])/g, ' $1').trim()} Module</span>
                      <label className="flex items-center cursor-pointer group">
                        <span className="text-xs text-gray-500 mr-2 font-medium">{mods.access ? 'ON' : 'OFF'}</span>
                        <div className={\`w-10 h-5 rounded-full p-0.5 transition-colors \${mods.access ? 'bg-indigo-600' : 'bg-gray-300'}\`}>
                          <input
                            type="checkbox"
                            className="hidden"
                            checked={!!mods.access}
                          ...
                          />
                          <div className={\`w-4 h-4 rounded-full bg-white shadow transform transition-transform \${mods.access ? 'translate-x-5' : 'translate-x-0'}\`} />
                        </div>
                      </label>
                    </div>
                    
                    {/* Collapsible body */}
                    <div className={\`p-4 space-y-3 \${!mods.access ? 'hidden' : ''}\`}>
...
c = c.replace(/\{Object\.entries\(permissions\)\.map\(\(\[moduleKey, mods\]\) => \([\s\S]*?<\label>\s*\);\s*\}\)\}\s*<\/div>\s*<\/div>\s*\)\)\}/, newRender.trim());
